...
SIMPLICITY IS THE HOUSE STYLE. The published MyPhonicsBooks below are the quality bar — notice how simple they are. One story thread in one connected location. Hero plus AT MOST one adult. AT MOST TWO key objects. Exactly ONE physical mechanism, so ordinary a parent could re-enact it in the kitchen. Never add depth, sub-plots, planted objects or extra beats: every added detail is another thing the illustrations can get wrong. DRAWABLE, NOT INVENTED (Lynden 2026-08-20): the mechanism must be a common, instantly recognisable childhood activity or object doing what it always does - something you could find a thousand photos of. NEVER invent a new toy, contraption or novel combination of objects (a ring threaded on a string between two people is an invention; a kite, a ball, a bucket on a rope, a paper boat are not). If an illustrator would need the mechanism explained, it is wrong. When an everyday object has a traditional, most-recognisable form, NAME that form in the briefs (a washing line is a rope stretched between two posts with wooden pegs - never a folding metal rack; a broom has a wooden handle and bristles). BRITISH ENGLISH VOCABULARY: use the British word always - 'the washing', never 'the wash'; 'trousers', never 'pants' as outerwear. SENTENCE SHAPE: never join two clauses with a comma plus 'and' - write two short sentences instead; young readers take them one at a time. POINTABLE BEATS (Lynden 2026-08-20, the paper-boat lesson): every story beat must change something a child could POINT AT between two pictures - sunk vs floating, wet vs dry, empty vs full, up vs down, here vs gone. A fix or a problem whose before/after differs only subtly (a slight lean, patting something flatter, a small adjustment) is INVISIBLE at picture-book scale and must not be written, however physically true it is. And a late complication is OPTIONAL: if the build or task simply succeeds, a social payoff - launching it, showing Mum, a friend coming to look - is a complete ending; never manufacture a wobble you cannot draw obviously.
...
PHONICS CONTRACT for Level ${LEVEL}: every word decodable using ONLY these graphemes: ${JSON.stringify(level.cumulative)} — or on this tricky list: ${JSON.stringify(level.trickyWords)}. The name "${child.name}" is always allowed. NATURAL LANGUAGE (Lynden 2026-08-20): always use the most common everyday word a five-year-old already says. If the natural word does not decode, REPHRASE the sentence - never reach for an obscure decodable word a child wouldn't know ('pads in' when you mean walks in; 'trots in' is fine because children know it). If the story truly needs ONE common word that does not decode (like 'walks'), nominate it as writer_tricky_word - it will be taught as a tricky word up front. At most one per book; leave it empty ("") if not needed. It still counts as allowed in the story text. HONEST SOUNDS: a word only counts as decodable if saying its taught letter-sounds actually produces the word children say. 'wash' comes out 'w-a-sh' but is said 'wosh'; 'basket' is said 'baskit'. Words like these are NOT decodable whatever the letters suggest - avoid them, or spend the writer_tricky_word on the one you truly need. And read_words must avoid split-digraph spellings (a_e, i_e, o_e, u_e, like 'smiles') - the Story Words page cannot yet mark the split sound and will dot it wrongly. Focus sound "${SOUND}" in at least 3 distinct words of the story text. read_words: EXACTLY 6 words that appear in the story text — EXACTLY 2 containing "${SOUND}", plus 4 other level-worthy words.
