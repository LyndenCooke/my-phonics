...
Known still-open (needs Lynden/pedagogy): word-by-word tricky-word classification display, "All Our Sounds So Far" completeness on the reference page, `with`/`th` Future-Sound-yet-required-in-story policy, and the green-words bank expansion (1,950 words; "problem" absent). The cold editor's `phonics_presentation` field flags contradictions but the underlying ledger/template logic is library-wide, not forge-specific.
