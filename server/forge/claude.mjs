...
Requirements:
- SPEND THE HERO'S NAME CAREFULLY. Sounded out honestly a name usually is not decodable at all, so the book teaches it as a tricky word — a story saying "${child.name}" eight times makes a child decode an undecodable word eight times and reads like a robot. Name the hero in the FIRST sentence, then use he, she, him, her or they and let the pictures carry who it is. AT MOST ONE use of the name per page after page 1, and never twice in one sentence. ("he", "she", "we", "me" and "be" are already approved tricky words at this level — use them.)
...
const SCENE_QA_SCHEMA = {
  ...
  required: ["named_objects", "action_shown", "object_states", "mechanism_legible", "distinguishing_feature", "character_match", "count_check", "state_assertions", "defect_sweep", "pass", "severity", "reason"],
  additionalProperties: false,
};
