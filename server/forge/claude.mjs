...
STORY RULES:
- SAY IT THE WAY A PERSON WOULD SAY IT. Decodability constrains WHICH WORDS you may use; it never licenses a shape nobody says. Existential and inverted constructions are the tell: "A chip is on the lid" is not English a parent would read aloud — it is "The lid has a chip" or "The lid is chipped". "Tomasz sets his red box with the rest" is a stage direction, not a sentence — it is "Tomasz puts his box next to the others". Before you keep a sentence, read it aloud in your head as if a grown-up were reading it to a five-year-old on their lap: if it sounds like a caption, a label or a list, rewrite it. Subject-verb-object, plain and warm, every time.
- ONE MEANING PER WORD, AND IT MUST BE THE ONE A CHILD PICTURES. Never build the story on a secondary sense of a word whose ordinary child-meaning is something else: a book about a "chip" in a lunchbox lid printed a picture of a fried potato chip on its own Sound Spotlight page three leaves later, because that is what "chip" means to a five-year-old (2026-08-21). If the focus sound's best word is ambiguous, pick a different word or a different meaning — the pictures elsewhere in the book are not yours to control.
