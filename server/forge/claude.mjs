        ...
        properties: {
          ...
          objects: {
            ...
            items: {
              ...
              properties: {
                name: { type: "string" },
                identity_lock: { type: "string", description: "The object's immutable build, repeated VERBATIM on every page where it appears: exact silhouette/geometry, proportions, component count, material, colour and fixed markings. Never include position, action or temporary state." },
                state: { type: "string" },
              },
              required: ["name", "identity_lock", "state"],
              additionalProperties: false,
            },
          },
...
5. CAMERA — direct like a real picture book, and use the camera to PROTECT consistency:
   - BEFORE choosing a shot, fill setting_id with one canonical PHYSICAL setting id and setting_relation with exactly one plan: "new-setting" only for the first appearance of that setting, "same-view" for an intentional repeated frame, "same-setting-new-angle" for another angle or part of an established setting, or "same-setting-closeup" for a crop within it. Writer location ids may describe positions along a route; do NOT copy a new id merely because the page moves to another part of the same cart, room, garden or street. All views within one physical setting share one setting_id. If the story leaves a setting and later RETURNS, reuse its earlier setting_id and select the appropriate same-setting relation; never attach the returning scene to the intervening room. This is a binding continuity plan made before painting.
...
9. OBJECTS ON THIS PAGE — fill "objects" with every physical thing a reader would actually SEE in this frame that the continuity of the book depends on, each with the state it is in ON THIS PAGE ("bowl — empty, waiting on the mat"; "date balls — do not draw, they do not exist yet, the mix is still wet paste"). Leave the list EMPTY if none are visible.
   IDENTITY AND STATE ARE SEPARATE. For each object, fill identity_lock with one complete immutable build specification: exact silhouette/geometry, proportions, number and arrangement of parts, material, colour and fixed markings. Copy that identity_lock VERBATIM on every later page where the object appears. Put location, orientation, contents, damage, open/closed and action only in state. A curved U-shaped track cannot become a long straight track because a later sentence says it "runs to" a bush: keep the exact fixed U geometry and orient that same object towards the bush, or expose the story/geometry conflict before painting. Never stretch, shorten, widen, add pieces to or remove pieces from a recurring object unless the story explicitly shows that physical change.
