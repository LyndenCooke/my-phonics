...
export default function PartnerCodeField() {
  ...
  const apply = async () => {
    ...
    if (!/^[A-Z0-9]{4,12}$/.test(code)) { setError(t('partnerCode.invalidFormat')); return; }
    setChecking(true);
    try {
      const { data, error: rpcErr } = await sbRpc('partner_code_discount', { p_code: code });
      if (rpcErr) { setError(t('partnerCode.checkFailed')); return; }
      if (typeof data === 'number' && data > 0) {
        ...
        setError(t('partnerCode.notRecognised'));
      }
    } finally {
      setChecking(false);
    }
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <p className="text-sm font-bold text-foreground truncate">
            <Trans
              t={t}
              i18nKey="partnerCode.applied"
              values={{ code: applied.code, pct: applied.pct }}
              components={{ code: <span dir="ltr" className="font-mono text-primary-ink" /> }}
            />
          </p>
        </div>
        <button onClick={remove} className="text-xs font-semibold text-muted-foreground hover:text-foreground inline-flex items-center gap-1 shrink-0">
          <X className="h-3.5 w-3.5" /> {t('partnerCode.remove')}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="mb-5 text-center">
      {!open ? (
        ...
        >
          <Ticket className="h-4 w-4" /> {t('partnerCode.prompt')}
        </button>
      ) : (
        <div
        ...
        >
          <div className="flex items-center gap-2">
            <input
              ...
              aria-label={t('partnerCode.placeholder')}
              dir="ltr"
            ...
            >
              {checking ? <Loader2 className="h-4 w-4 animate-spin" /> : t('partnerCode.apply')}
            </button>
          </div>
          {error && <p className="text-xs text-red-500 mt-2 text-start px-1">{error}</p>}
        </div>
      )}
    </div>
  );
