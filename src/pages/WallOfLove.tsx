    ...
    <Layout>
      <div className="px-5 lg:px-8 pt-8 lg:pt-12 pb-14 max-w-6xl mx-auto">
        ...
        >
          ...
          >
            <Heart className="w-3.5 h-3.5 fill-current" /> {t('badge')}
          </span>
          ...
          {avgRating && items && items.length > 2 && (
            <p className="mt-3 text-sm lg:text-base text-muted-foreground font-bold inline-flex items-center gap-1.5">
              ...
              {t('rating', { avg: avgRating, count: items.length })}
            </p>
          )}
        </motion.div>
        ...
        {items === null ? (
          ...
          <div className="mt-12 text-center max-w-sm mx-auto">
            ...
            <p className="font-display text-xl font-extrabold text-foreground mt-3">{t('empty.title')}</p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map(n => (
                      <Star key={n} className={`w-4 h-4 ${(item.rating ?? 5) >= n ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground/25'}`} />
                    ))}
                  </div>
                  <blockquote dir="auto" className="text-[15px] text-foreground leading-relaxed mt-3">
                    “{item.quote}”
                  </blockquote>
        ...
        >
          <h2 className="font-display text-xl lg:text-2xl font-extrabold text-foreground">
            {user ? t('cta.addStory') : t('cta.ownStory')}
          </h2>
          <div className="mt-5 flex flex-col sm:flex-row items-center justify-center gap-3">
            {user ? (
              ...
              >
                <PenLine className="w-[18px] h-[18px]" /> {t('cta.leaveReview')}
              </button>
            ) : (
              <button
                ...
                style={{ background: '#E84B8A', boxShadow: '0 5px 0 #BE1862, 0 14px 28px -10px #E84B8A80' }}
              >
                <Sparkles className="w-[18px] h-[18px]" /> {t('cta.tryCheck')}
              </button>
            )}
            <button
