...
export function StudentPathwayStatus({
  ...
  return (
    <div className="text-sm">
      <div className="font-bold text-slate-900">{name}</div>
      <div className="text-slate-600" dir="ltr" lang="en">L{level} {LEVEL_NAME[level]}</div>
      {pos.block && !pos.block.isReview && (
        <div className="text-xs text-slate-500 mt-0.5">
          {t('pathwayPieces.blockTitle', { n: pos.block.blockNumber, total: pos.block.totalTeachingBlocks })}{' '}
          <span dir="ltr" lang="en">{pos.block.focusLabel}</span>
        </div>
      )}
      {pos.block?.isReview && (
        <div className="text-xs text-amber-700 font-semibold mt-0.5">{t('levelReview')}</div>
      )}
      {!compact && pos.current && (
        <div className="text-xs text-slate-500 mt-0.5">
          {t('learner.currentStep')} <span dir="ltr" lang="en" className="font-semibold text-slate-700">{pos.current.title}</span>
        </div>
      )}
      {!compact && pos.next && (
        <div className="text-xs text-slate-500">
          {t('learner.nextResource')} <span dir="ltr" lang="en" className="font-semibold text-slate-700">{pos.next.title}</span>
        </div>
      )}
      <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold mt-1 ${JUDGEMENT_TONE[pos.judgement]}`}>
        {t(`judgement.${pos.judgement}`, { defaultValue: JUDGEMENT_LABEL[pos.judgement] })}
      </div>
    </div>
  );
}
...
export function GroupPathwayStatus({
  ...
  if (!block) {
    return (
      <div className="text-xs text-slate-500"><span dir="ltr" lang="en">L{level} {LEVEL_NAME[level]}</span> · {t('learner.children', { count: childCount })}</div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3" data-school-level={level}>
      <div className="flex items-center justify-between">
        <span className="font-bold text-sm" dir="ltr" lang="en">L{level} {LEVEL_NAME[level]}</span>
        <span className="text-xs font-semibold text-slate-500">{t('learner.children', { count: childCount })}</span>
      </div>
      <div className="text-xs text-slate-500 mt-0.5">
      ...
      {today && (
        <div className="flex items-center gap-1.5 mt-2 text-xs">
          <span className="text-slate-400 font-semibold uppercase text-[10px] tracking-wider w-14">{t('learner.today')}</span>
          <ResourceTypeBadge type={today.kind as never} />
          <span dir="ltr" lang="en" className="font-semibold text-slate-700 truncate">{today.title}</span>
        </div>
      )}
      {next && (
        <div className="flex items-center gap-1.5 mt-1 text-xs">
          <span className="text-slate-400 font-semibold uppercase text-[10px] tracking-wider w-14">{t('learner.next')}</span>
          <ResourceTypeBadge type={next.kind as never} />
          <span dir="ltr" lang="en" className="font-semibold text-slate-700 truncate">{next.title}</span>
        </div>
      )}
    </div>
  );
