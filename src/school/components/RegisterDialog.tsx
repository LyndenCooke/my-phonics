...
export default function RegisterDialog({
  ...
  // Lesson names are stored in the DB (and used as a lookup key), so the
  // default stays English whatever language the teacher is using.
  ...
  const save = async () => {
    ...
    setSaving(false);
    if (error) {
      toast({ title: t('register.saveFailed'), description: (error as { message?: string }).message, variant: 'destructive' });
      return;
    }
    toast({ title: t('register.saved'), description: t('register.savedBody', { present: presentCount, total: students.length, date: new Date(date).toLocaleDateString(i18n.language) }) });
    onSaved?.();
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-start justify-between p-5 border-b border-slate-100">
          <div>
            <h3 className="font-bold text-xl">{t('register.title')}</h3>
            <p className="text-sm text-slate-500">{title}</p>
          </div>
          <button onClick={onClose} aria-label={t('register.close')} className="text-slate-400 hover:text-slate-900"><X className="w-5 h-5" /></button>
        </header>

        <div className="px-5 py-3 border-b border-slate-100 grid grid-cols-2 gap-3">
          <label className="block">
            <span className="block text-xs font-bold text-slate-600 mb-1">{t('register.date')}</span>
          ...
          <label className="block">
            <span className="block text-xs font-bold text-slate-600 mb-1">{t('register.lesson')}</span>
        ...
        <div className="px-5 py-2 border-b border-slate-100 flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-500">{t('register.present', { present: presentCount, total: students.length })}</span>
          <div className="flex gap-2">
            <button onClick={() => markAll('present')} className="text-xs font-semibold text-emerald-700 hover:underline">{t('register.allPresent')}</button>
            <button onClick={() => markAll('absent')} className="text-xs font-semibold text-rose-700 hover:underline">{t('register.allAbsent')}</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {loading ? (
            ...
            <p className="text-sm text-slate-500 text-center py-8">{t('register.empty')}</p>
          ) : (
                ...
                return (
                  <li key={s.id} className="flex items-center justify-between gap-2 py-1">
                    ...
                    <div className="flex gap-1">
                      <SegBtn active={st === 'present'} tone="emerald" onClick={() => setOne(s.id, 'present')}><Check className="w-3.5 h-3.5" /> {t('register.statusPresent')}</SegBtn>
                      <SegBtn active={st === 'late'} tone="amber" onClick={() => setOne(s.id, 'late')}><Clock className="w-3.5 h-3.5" /> {t('register.statusLate')}</SegBtn>
                      <SegBtn active={st === 'absent'} tone="rose" onClick={() => setOne(s.id, 'absent')}><X className="w-3.5 h-3.5" /> {t('register.statusAbsent')}</SegBtn>
                    </div>
                  </li>
                );
              })}
        ...
        <footer className="p-5 border-t border-slate-100 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-slate-300 text-sm font-semibold hover:bg-slate-50">{t('common:actions.cancel')}</button>
          <button onClick={save} disabled={saving || students.length === 0}
            ...
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : t('register.save')}
          </button>
        </footer>
      </div>
    </div>
