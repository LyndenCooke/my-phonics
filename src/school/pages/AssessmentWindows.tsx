...
type Student = { id: string; classroom_id: string; first_name: string; last_name: string | null; current_level: string | null };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: rooms }, { data: stu }, { data: ass }] = await Promise.all([
        ...
        schoolDb.students().select('id, classroom_id, first_name, last_name, current_level').eq('school_id', school.id),
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-bold uppercase mb-1">
              <ClipboardCheck className="w-3.5 h-3.5" /> Current window — open
            </div>
            <h2 className="font-display text-xl font-extrabold">Summer half-term assessment window</h2>
            <p className="text-sm text-slate-500">{fmt(windowStart)} – {fmt(windowEnd)} · all classes included</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-extrabold">{model.totalAssessed}<span className="text-slate-400 text-lg">/{model.total}</span></div>
            <div className="text-xs text-slate-500 uppercase tracking-wider">assessed this window</div>
          </div>
        </div>
      ...
      {/* Regrouping outcomes */}
      <section>
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-3">Regrouping after this window</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <p className="text-sm text-slate-600 mb-4">
          {model.total} children across {model.perClass.length} classes. {model.totalAssessed} assessed this window,
