...
export default function SchoolLevels() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-display text-3xl font-extrabold tracking-tight mb-2">{t('levels.title')}</h1>
        <p className="text-slate-600">{t('levels.intro')}</p>
      </header>
            ...
            >
              <header className="s-bg-level text-white px-5 py-3 flex items-baseline justify-between">
                ...
                <div className="text-end text-xs opacity-90">
                  <div>{t('ages', { range: lvl.ageRange.replace(/^Ages\s*/, '') })}</div>
                  <div className="mt-0.5" dir="ltr" lang="en">{lvl.phaseLabel}</div>
                </div>
              </header>

              <div className="p-5 grid gap-4 md:grid-cols-2">
                <Field label={t('levels.decodableRule')} full>{t(`levels.rules.${lvl.level}`, { defaultValue: lvl.decodableRule })}</Field>
                <Field label={t('levels.sentenceLength')}>{t(`levels.sentenceLengths.${lvl.level}`, { defaultValue: lvl.sentenceLength })}</Field>

                <Field label={t('levels.gpcsTaught')} full>
                  <div dir="ltr" lang="en" className="flex flex-wrap gap-1.5 mt-1">
                    {lvl.gpcs.map((g) => (
                ...
                <Field label={t('levels.newTricky')} full>
                  <div dir="ltr" lang="en" className="flex flex-wrap gap-1.5 mt-1">
              ...
              <div data-school-level={lvl.level} className="px-5 py-3 border-t border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <ResourceCount label={t('levels.storybooks')} value={storybooks.length} />
                <ResourceCount label={t('levels.soundBooks')} value={soundBooks.length} />
                <ResourceCount label={t('levels.blendingBooks')} value={blendingBooks.length} />
                <ResourceCount label={t('levels.worksheetPacks')} value={WORKSHEET_PACKS.reduce((n, p) => n + p.countPerLevel(lvl.level), 0)} />
              </div>
              ...
              <div className="px-5 pb-5 space-y-4">
                ...
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">{t('levels.worksheetComponents')}</h3>
                    ...
                    {WORKSHEET_COMPONENTS.map((c) => {
                      ...
                      return (
                        <div key={c.id} className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-slate-700">{t(`worksheets.components.${c.id}.label`, { defaultValue: c.label })}</span>
                        </div>
                      );
                    })}
                  </div>
...
function ResourceList({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      ...
      <ul dir="ltr" lang="en" className="text-sm text-slate-700 space-y-0.5 text-left">
        {items.map((it) => (
