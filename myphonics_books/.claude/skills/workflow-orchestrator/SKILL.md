...
- [ ] Inject eye-style reference image when generating heroes (use approved hero from any previous book)
- [ ] Use hex colour codes for skin and hair (NEVER vague descriptions)
- [ ] Show hero images to user for approval BEFORE generating scene images
...
## STEP 3a: Object Identification + Secondary Character Lock + Setting Bible

**Skill:** `.claude/skills/illustration-director/SKILL.md`
**Input:** Approved story
**Output:** THREE locked references — objects, secondary characters, setting bible

### What Happens:

**A. Recurring Objects:**
- Identify all objects mentioned more than once in the story
- Define EXACT visual appearance for each (colour, style, size, material)
- These definitions are copy-pasted IDENTICALLY into every prompt where the object appears

**B. Secondary Characters (NEW — 2026-03-18):**
- Identify all NON-HERO characters who appear in more than one scene (grandparent, parent, sibling, teacher, friend)
- Define EXACT appearance for each: age look, skin tone (hex code), hair (colour, style), full outfit with specific colours and style of any headwear (turban colour, hijab colour, hat style etc.)
- Lock this into a one-sentence description that is copy-pasted IDENTICALLY into every prompt where they appear
- **Critical for headwear:** turbans, hijabs, hats, scarves must have the EXACT colour and style specified so they don't drift between pages

**C. Setting Bible (NEW — 2026-03-18):**
- Write ONE locked description of the book's primary background environment
- Include: location type, architecture style and colour, key visible elements, time of day/season, colour palette
- This full phrase is copy-pasted IDENTICALLY into every scene prompt
- If the story moves locations, write a separate Setting Bible for each location

---

## STEP 3b: Reference Image Generation + Injection Plan

**Skill:** `.claude/skills/illustration-director/SKILL.md`
**Input:** Story + character + secondary char list + object list + setting bible + Cultural Brief
**Output:** All reference images + per-scene injection plan table

### What Happens:
1. **Generate main hero reference** (text-to-image with eye-style reference injected)
2. **Generate side character hero references** — one per named character appearing on 2+ pages
3. **Generate background plates** — one per distinct location used on 2+ pages (NO characters)
4. **Generate object references** — one per recurring prop appearing on 2+ pages
5. **Show ALL references to user for approval** before any scene images
6. **Produce Injection Plan table** — maps every scene to exactly which references it requires

### Checkpoint:
✅ Main hero approved by user?
✅ All side character heroes approved by user?
✅ All background plates approved by user?
✅ All object references approved by user?
✅ Injection Plan table produced?
✅ Eye style correct in all hero references (solid black dots, no white)?
✅ Cultural Brief compliance in all references?
✅ Modesty standards met?

**If ANY reference fails → Fix and regenerate before proceeding**

## STEP 3c: Scene Prompts

**Skill:** `.claude/skills/illustration-director/SKILL.md`
**Input:** Injection plan + all references + story text
**Output:** One scene prompt per image (cover + all story pages)

### What Happens:
- Write one prompt per scene guided by the injection plan
- Each prompt specifies which references to inject (main hero, side chars, background, objects)
- Action in each prompt matches the story text exactly
- All prompts include eye style, art style, modesty, and "no text" requirements

### Checkpoint:
✅ Action in each prompt matches story text word-for-word?
✅ Every required reference listed per injection plan?
✅ "No text, words, or letters" in every prompt?
✅ Cultural Brief details present?
...
**Script:** Per-book script (e.g. `scripts/generate_l3_3_images.py`)
**Input:** Hero prompt + scene prompts + eye reference + hex colours
**Output:** PNG files (hero_reference.png, cover.png, page1-8.png)
...
2. **Modesty:** All characters fully covered, dresses have leggings, no bare legs
3. **Safety:** No children with knives/sharp objects, adults handle dangerous tasks
4. **Main character consistency:** Same face, hair, outfit across ALL pages
5. **Secondary character consistency:** Grandparent/parent/sibling/friend same outfit, same headwear colour and style, same hair in EVERY appearance
6. **Object consistency:** Recurring objects identical in every appearance (same colour, shape, size)
7. **Background/setting consistency:** Same architecture, colour palette, viewpoint across pages set in the same location
8. **Cultural accuracy:** Clothing, architecture, objects match Cultural Brief
9. **Action/text match:** Each image matches its story page text exactly
10. **Art style:** Consistent whimsical children's book style throughout

### Checkpoint:
✅ Every image viewed with Read tool?
✅ Eyes correct in ALL images (no white)?
✅ Modesty maintained in ALL images?
✅ Safety standards met?
✅ MAIN character consistent across all pages?
✅ SECONDARY characters consistent across all their appearances?
✅ Objects consistent across pages?
✅ Backgrounds consistent across same-location pages?
✅ Cultural Brief compliance?

**If FAIL → Identify failing images, update prompts or generate missing references, regenerate ONLY failures, re-verify**
...
| Image eyes wrong | Step 4 | Strengthen eye prompt, re-inject eye reference, regenerate all scenes |
...
| Main character inconsistent | Step 3b + Step 4 | Regenerate hero; regenerate affected scenes |
| Secondary character inconsistent | Step 3b + Step 4 | Generate side character hero reference (if missing); regenerate affected scenes |
| Object inconsistency | Step 3a + Step 3b + Step 4 | Generate object reference image; update injection plan; regenerate affected scenes |
| Background/setting inconsistent | Step 3a + Step 3b + Step 4 | Generate background plate; update injection plan; regenerate affected scenes |
